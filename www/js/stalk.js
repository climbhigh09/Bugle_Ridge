...
(function () {
  ...
  const SPEED = 2.8; // px per real second on open ground; 1 real second = 1 in-game minute
  ...
  function genMap(area, seed, elkC) {
    ...
    const m = area.mix;
    const t1 = quantile(v1, 1 - m.timber);
    const t2 = quantile(v2, 1 - m.deadfall / Math.max(0.05, 1 - m.timber));
    const t3 = quantile(v3, 1 - m.shale / Math.max(0.05, 1 - m.timber - m.deadfall));
  ...
  // Hillshaded topo: light from the NW, faint contours, individual spruce crowns with shadows, timber shading the ground to its SE.
  function renderMap(grid, seed) {
    const c = document.createElement('canvas'); c.width = W; c.height = H;
    const o = c.getContext('2d'), img = o.createImageData(W, H), d = img.data;
    const C = {
      tBase: hex('#17271b'), tCrown: hex('#2e4c33'), tMid: hex('#213a26'), tShadow: hex('#0c150e'),
      o1: hex('#52643f'), o2: hex('#4a5b3a'), oDry: hex('#6b6a45'), oTuft: hex('#687c4b'),
      dBase: hex('#4b5034'), dLog: hex('#9a907c'), dLogSh: hex('#2e2a1f'),
      s1: hex('#5f5e59'), s2: hex('#88857c'), s3: hex('#42413d')
    };
  ...
  function mapFor(e) {
    ...
    if (c && c.seed === e.map.seed) return c;
    const grid = genMap(BR.AREAS[e.area], e.map.seed, { x: e.map.ex, y: e.map.ey });
    return (BR.planCache = { seed: e.map.seed, grid, canvas: renderMap(grid, e.map.seed) });
  }

  function elkTop(g, x, y, dir, bull, z, headUp) {
    const f = dir > 0;
    R(g, x - 2 * z, y - z, 4 * z, 2 * z, P.hide);
    R(g, f ? x - 2 * z : x + z, y - z, z, 2 * z, P.rump);
    R(g, f ? x + 2 * z : x - 3 * z, y - (headUp ? z : 0), z, z, P.mane);
    if (bull) { R(g, f ? x + 2 * z : x - 3 * z, y - 2 * z, z, z, P.antler); R(g, f ? x + 1 * z : x - 2 * z, y - 2 * z, z, z, P.antler); }
  }
  ...
  function routeStats(grid, area, e, startClock) {
    ...
    for (let s = 0; s < pts.length - 1; s++) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        total += step; if (ter.cover < 0.5) open += step; noise += ter.noise * step; minutes += step / (SPEED * ter.speed);
        if (scentAt < 0) {
          const sc = BR.scent(area, startClock + minutes / 60), dx = ec.x - x, dy = ec.y - y, d = Math.hypot(dx, dy) || 1;
          if (d < 83 && (sc.x * dx + sc.y * dy) / d > 0.72) scentAt = s + 1;
        }
      }
    }
    const last = pts[pts.length - 1];
    return {
      cover: total ? Math.round(100 * (1 - open / total)) : 0, noise: total ? noise / total : 0,
      minutes: Math.round(minutes), arrive: startClock + minutes / 60,
      endYd: Math.round(Math.hypot(last.x - ec.x, last.y - ec.y) * YD), scentAt
    };
  }
  ...
  BR.scenes.plan = {
    enter() {
      ...
      if (!e.map) {
        ...
        e.map = { seed, ex, ey };
        e.elk = e.target.elk.map(k => ({ x: ex + (r() - 0.5) * 22, y: ey + (r() - 0.5) * 12, bull: k.bull, hideAt: k.hideAt, dx: r() < 0.5 ? 1 : -1, up: false }));
        let lead = e.elk.findIndex(k => !k.bull); if (lead < 0) lead = 0;
    ...
    draw(g) {
      const S = BR.S, e = S.enc, has = !!S.items.windChecker;
      g.drawImage(this.m.canvas, 0, 0);
      e.elk.forEach(k => elkTop(g, Math.round(k.x), Math.round(k.y), k.dx, k.bull, 1, false));
      ...
      if (has) {
        const st = routeStats(this.m.grid, this.area, e, S.clock);
        const sc = BR.scent(this.area, st.arrive), last = pts[pts.length - 1];
        BR.line(g, last.x, last.y, last.x + sc.x * 40, last.y + sc.y * 40, P.wind, 2);
        windBox(g, sc, P.wind);
      } else {
        const c = BR.conditions(), v = BR.DIRS[c.from];
        windBox(g, { x: -v[0], y: -v[1] }, P.bone);
      }
    },
    up(p) {
    ...
    hud() {
      ...
      const noiseBars = Math.round(BR.clamp(st.noise, 0, 1.25) * 4);
      let scent;
      if (has) scent = st.scentAt > 0 ? `<span class="bad">Scent reaches them near waypoint ${st.scentAt}</span>` : '<span class="ok">Scent stays clear of them</span>';
      else scent = `<span class="dim">Wind from ${c.from} ${c.mph} · sun on slope ${BR.fmt(this.area.sun)}</span>`;
      const has1 = e.route.length > 0;
      BR.hud(`
        ...
        <div class="row sm"><span>${has1 ? `Ends ${st.endYd} yd from elk · arrive ${BR.fmt(st.arrive)}` : 'Tap the map to drop up to 4 waypoints.'}</span></div>
  ...
  BR.scenes.stalk = {
    enter(a) {
      ...
      this.holding = false; this.done = false; this.flashT = 0; this.flashMsg = ''; this.hudT = 0; this.saveT = 5;
    ...
    tick(dt) {
      const S = BR.S, e = S.enc, st = e.st;
      if (this.paused || this.done) return false;
      // Frozen and nothing watching you: stop the clock and the frame loop until the thumb comes back.
      if (!this.holding) {
      ...
      const here = TER[terrainAt(this.m.grid, st.hx, st.hy)];
      let moving = false;
      if (this.holding) {
        let target = null;
        ...
        if (target) {
          const dx = target.x - st.hx, dy = target.y - st.hy, L = Math.hypot(dx, dy), step = SPEED * here.speed * dt * (st.seg < route.length - 1 ? 1 : 0.6);
      ...
      if (moving && ld < 100 && Math.random() < here.noise * 0.28 * dt) {
      ...
      if (ld < 83 && dot > 0.72) {
        ...
        if (st.scent > 0.25 && !st.warned) { st.warned = true; this.flash('A cow’s nose is up. Wind’s wrong.'); BR.vibe(30); }
    ...
    draw(g) {
      ...
      const z = n && n.px < 30 ? 3 : n && n.px < 75 ? 2 : 1;
      ...
      for (const k of e.elk) {
        if (k.gone) continue;
        const [x, y] = T(k.x, k.y);
        elkTop(g, x, y, k.dx, k.bull, z, k.lead ? st.look : k.up);
      ...
      if (z > 1) BR.ring(g, hx, hy, (Math.max(...BR.bow().pins) / YD) * z, 'rgba(227,166,70,.35)', 6);
    ...
    hud() {
      const S = BR.S;
      BR.hud(`
        <div class="row"><span class="t">STALKING</span><span class="hi" id="st-yd"></span></div>
        <div class="row"><span>LEAD COW</span><span id="st-meter"></span></div>
        <div class="row sm"><span id="st-msg"></span><span class="dim" id="st-clock"></span></div>
        ${S.items.windChecker ? '<div class="row sm"><span>Scent: <span id="st-scent"></span></span></div>' : ''}
        <div class="sp"></div>
    ...
    upd() {
      ...
      const lvl = Math.ceil(st.alert / 20);
      $('st-meter').innerHTML = BR.meter(lvl, 5, st.alert > 60 ? 'bad' : st.alert > 25 ? 'warn' : '');
      $('st-clock').textContent = BR.fmt(S.clock);
      const route = [START].concat(e.route);
      let msg, cls = 'dim';
      if (this.paused) { msg = 'Holding still. Hold to creep.'; cls = 'dim'; }
      else if (this.flashT > 0) { msg = this.flashMsg; cls = 'toast'; }
      else if (st.look && st.alert > 60) { msg = 'She’s staring at you. Freeze.'; cls = 'bad'; }
      else if (st.look) { msg = 'Her head’s up.'; cls = 'hi'; }
      else if (st.seg >= route.length - 1) msg = 'End of route. Hold to close in.';
      else msg = 'She’s feeding.';
      ...
      const b = $('st-shoot'), inRange = n.yd <= 80;
      b.disabled = !inRange;
    ...
    act(a) {
      const S = BR.S, e = S.enc, st = e.st;
      if (a === 'shoot') {
        const live = e.elk.filter(k => !k.gone).map(k => ({ k, yd: Math.round(Math.hypot(k.x - st.hx, k.y - st.hy) * YD) })).filter(o => o.yd <= 80);
        if (!live.length) return;
        live.sort((a2, b2) => (b2.k.bull - a2.k.bull) || (a2.yd - b2.yd));
        const r = Math.random();
        e.shot = { range: live[0].yd, bull: live[0].k.bull, angle: r < 0.5 ? 'broadside' : r < 0.72 ? 'quartering-away' : r < 0.93 ? 'quartering-to' : 'facing', from: 'stalk', alert: st.alert };
        this.done = true;
        BR.go('shot');
      } else if (a === 'call') {
        const n = this.nearest();
        this.done = true;
        BR.go('call', { area: e.area, dist: n ? n.yd : 150, bull: e.elk.some(k => k.bull && !k.gone), fromStalk: true });
      }
    },
    back() {
      this.holding = false;
